    ...
    <form action={actionState}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Your birthdate will not be displayed publicly.</CardDescription>
        </CardHeader>
        <CardContent>
...
function SaveButton({ newBirthdate, oldBirthdate }: SaveButtonProps) {
  ...
  return (
    <Button
      type='submit'
      size='sm'
      disabled={pending || (oldBirthdate && newBirthdate && isSameDay(newBirthdate, oldBirthdate))}
    >
      Save changes
    </Button>
  )
